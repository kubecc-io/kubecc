...
var SleepCmd = &cobra.Command{
	...
	Long: `The sleep tool will sleep for the given duration, then exit 
with a 0 status code (unless given invalid arguments).
...
func worker(
	...
	for {
		...
		lg.Infof("[%0*d/%0*d] Running task",
			...
			counterLen, numTasks)
		d := time.Duration(rand.Int63n(int64(max-min)) + int64(min)).
			Round(granularity)
...
func init() {
	MakeSleepCmd.Flags().DurationVar(&max, "max", 4*time.Second,
		"Maximum duration to sleep for (exclusive)")
	MakeSleepCmd.Flags().DurationVar(&min, "min", 1*time.Second,
		"Minimum duration to sleep for (inclusive)")
	MakeSleepCmd.Flags().DurationVarP(&granularity, "granularity", "g", 100*time.Millisecond,
