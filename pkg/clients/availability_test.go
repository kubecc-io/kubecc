...
var _ = Describe("Status Manager", func() {
	...
	numListeners := 1
	It("should ensure component availability", func() {
		experiment := gmeasure.NewExperiment("availability checker latency")
		for iteration := 0; iteration < 10; iteration++ {
			...
			experiment.MeasureDuration("connect latency", func() {
				Eventually(completed.Load).Should(Equal(int32(0)))
			})
			for _, ch := range available {
			...
			experiment.MeasureDuration("disconnect latency", func() {
				Eventually(completed.Load).Should(Equal(int32(0)))
			})
			for _, ch := range unavailable {
				Expect(ch).NotTo(Receive())
